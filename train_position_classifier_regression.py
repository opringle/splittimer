...
def main():
    parser = argparse.ArgumentParser(description="Train a position regressor on preprocessed video clip data.")
        ...
        if (epoch + 1) % args.eval_interval == 0 or epoch == args.num_epochs - 1:
            if val_loader:
                ...
                if total_samples > 0:
                    ...
                    r2 = 1 - (torch.sum((all_preds - all_labels)**2) / torch.sum((all_labels - all_labels.mean())**2))
                    logging.info(f'Epoch {epoch+1}, Val Loss (MSE): {val_loss:.4f}, MAE: {mae.item():.4f}, R2: {r2.item():.4f}')
                    writer.add_scalar('Loss/val', val_loss, epoch)
                    writer.add_scalar('MAE/val', mae.item(), epoch)
                    writer.add_scalar('R2/val', r2.item(), epoch)
                else:
                    logging.warning(f'Epoch {epoch+1}: No validation samples')
            else:
