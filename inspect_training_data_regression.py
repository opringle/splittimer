...
def main():
    parser = argparse.ArgumentParser(description="Inspect training data by generating an HTML page with random samples per sample type.")
    parser.add_argument("csv_path", type=str, help="Path to the training data CSV")
    parser.add_argument("--num_samples", type=int, default=3, help="Number of samples to display per sample type")
    ...
    # Sample N samples per sample_type
    sampled_df = pd.concat([group.sample(min(len(group), args.num_samples)) for _, group in df.groupby('sample_type')])
    ...
    # Group by sample_type for display
    for sample_type, group in sampled_df.groupby('sample_type'):
        html += f"<h2>Sample Type: {sample_type}</h2>"
        for row in group.itertuples():
            ...
            # Add to HTML with track_id, sample_type, and continuous label
            html += f"""
            <div class="sample">
            <h3>Track ID: {track_id}, Sample Type: {sample_type}, Label: {label:.2f}, {v1_rider_id} frame {v1_frame_idx} and {v2_rider_id} frame {v2_frame_idx}</h3>
