...
def generate_negative_samples(v1_idx, positive_v2_idx, v2_total_frames, clip_length, max_negatives, metadata_base, v2_min_idx, v2_frame_idx_to_split_number=None, split_number=None):
    if v2_min_idx > v2_total_frames - 1:
        logging.warning(f"Cannot generate negative samples: v2_min_idx {v2_min_idx} > v2_total_frames - 1 {v2_total_frames - 1}")
        return [], [], []
...
def generate_training_samples(v1_indices, v1_labels, v1_rider_id, v1_track_id,
    ...
    for split_number in range(start_split, len(v1_split_indices)):
        ...
        negative_labels, negative_indices, negative_metadata = generate_negative_samples(
            v1_split_idx, v2_split_idx, v2_total_frames, clip_length, max_negatives_per_positive, 
            metadata_base, v2_min_idx, v2_frame_idx_to_split_number, split_number
        )
    ...
    for seg in range(num_segments):
        ...
        for idx1 in selected_idx1:
            fraction_through_v1_segment = (idx1 - v1_start_seg) / (v1_end_seg - v1_start_seg)
            idx2_float = v2_start_seg + fraction_through_v1_segment * (v2_end_seg - v2_start_seg)
            ...
            sample_metadata.append({**metadata_base, 'sample_type': 'augmented'})
            
            # Generate negative samples for augmented positives
            negative_labels, negative_indices, negative_metadata = generate_negative_samples(
                idx1, idx2, v2_total_frames, clip_length, max_negatives_per_positive, 
                metadata_base, v2_min_idx
            )
            sample_labels.extend(negative_labels)
            sample_indices.extend(negative_indices)
            sample_metadata.extend(negative_metadata)
