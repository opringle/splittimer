...
def load_source_splits(config_path, track_id, source_rider_id):
    ...
    if not matching_videos:
        logging.error(
            f"No configuration found for track {track_id} and rider {source_rider_id} in {config_path}")
        exit(1)

    if len(matching_videos) > 1:
        logging.warning(
            f"Multiple videos found for track {track_id} and rider {source_rider_id}. Using the first one.")
    ...
    if not splits:
        logging.warning(
            f"No splits found for track {track_id} and rider {source_rider_id}")
...
def main():
    parser = argparse.ArgumentParser(
        description="Find corresponding splits in target video based on source video splits.")
    ...
    parser.add_argument('--targetRiderId', type=str,
                        required=True, help='Target rider identifier')
    ...
    parser.add_argument('--stride', type=int, default=1,
                        help='Stride for generating candidate split points in the target video')
    parser.add_argument('--threshold', type=float, default=0.5,
                        help='Threshold for considering a target clip as a split')
    parser.add_argument("--log-level", type=str, default="INFO",
                        choices=["DEBUG", "INFO", "WARNING", "ERROR", "CRITICAL"])
    parser.add_argument('--add_position_feature', action='store_true',
                        default=False, help='Add position feature to clips')
    parser.add_argument('--add_percent_completion_feature', action='store_true',
                        default=False, help='Add percent completion feature to clips')
    args = parser.parse_args()

    setup_logging(args.log_level)

    # Set up directories
    source_video_dir = get_video_file_path(args.trackId, args.sourceRiderId)
    target_video_dir = get_video_file_path(args.trackId, args.targetRiderId)
    ...
    source_timestamps = load_source_splits(
        args.config_path, args.trackId, args.sourceRiderId)
    logging.info(
        f"Loaded {len(source_timestamps)} source splits from {args.config_path} for track {args.trackId} and rider {args.sourceRiderId}")

    # Convert timestamps to frame indices in the source video
    source_video_path = Path(source_video_dir) / \
        f"{args.trackId}_{args.sourceRiderId}.mp4"
    source_fps, source_total_frames = get_video_fps_and_total_frames(
        source_video_path)
    source_end_indices = [timecode_to_frames(
        ts, source_fps) for idx, ts in enumerate(source_timestamps) if idx != 0]
    logging.debug(f"source_end_indices={source_end_indices}")

    # Determine total frames in target video
    target_video_path = Path(target_video_dir) / \
        f"{args.trackId}_{args.targetRiderId}.mp4"
    ...
    candidate_end_indices = list(
        range(args.F - 1, target_total_frames, args.stride))
    logging.info(
        f"Generated {len(candidate_end_indices)} candidate split points with stride {args.stride}")
    ...
    for source_end_idx in source_end_indices:
        ...
        features = load_image_features_from_disk(
            args.trackId, args.sourceRiderId, start_idx, source_end_idx, args.image_feature_path)
        ...
        source_samples[source_end_idx] = torch.from_numpy(
    ...
    for end_idx in tqdm(candidate_end_indices, desc="Processing target frames"):
        ...
        start_idx = indices[0]
        features = load_image_features_from_disk(
            args.trackId, args.targetRiderId, start_idx, end_idx, args.image_feature_path)
    ...
    logging.info(
        f"Saved {len(predicted_splits)} predicted splits to {args.output_file}")
