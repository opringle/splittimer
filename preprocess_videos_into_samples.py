    ...
    for row in tqdm(df.itertuples(), total=len(df), desc="Generating samples"):
        if args.feature_type == 'individual':
            ...
            if args.add_position_feature:
                v1_features = np.concatenate([v1_features, np.array(v1_indices, dtype=np.float32)[:, None]], axis=1)
                v2_features = np.concatenate([v2_features, np.array(v2_indices, dtype=np.float32)[:, None]], axis=1)  # Fixed typo here
            ...
            if args.add_percent_completion_feature:
                ...
                _, total_frames_v1 = get_video_fps_and_total_frames(video_path_v1)
                _, total_frames_v2 = get_video_fps_and_total_frames(video_path_v2)
                v1_percent = np.array(v1_indices, dtype=np.float32) / total_frames_v1
                v2_percent = np.array(v2_indices, dtype=np.float32) / total_frames_v2
                v1_features = np.concatenate([v1_features, v1_percent[:, None]], axis=1)
                v2_features = np.concatenate([v2_features, v2_percent[:, None]], axis=1)
            ...
            if args.add_position_feature:
                v1_features = np.concatenate([v1_features, np.array([row.v1_frame_idx], dtype=np.float32)[:, None]], axis=1)
                v2_features = np.concatenate([v2_features, np.array([row.v2_frame_idx], dtype=np.float32)[:, None]], axis=1)  # Fixed typo here
            ...
            if args.add_percent_completion_feature:
                ...
                _, total_frames_v1 = get_video_fps_and_total_frames(video_path_v1)
                _, total_frames_v2 = get_video_fps_and_total_frames(video_path_v2)
                v1_percent = np.array([row.v1_frame_idx / total_frames_v1], dtype=np.float32)
                v2_percent = np.array([row.v2_frame_idx / total_frames_v2], dtype=np.float32)
                v1_features = np.concatenate([v1_features, v1_percent[:, None]], axis=1)
                v2_features = np.concatenate([v2_features, v2_percent[:, None]], axis=1)
