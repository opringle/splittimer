...
def main():
    parser = argparse.ArgumentParser(description="Find corresponding splits in target video based on source video splits using a regression model.")
    ...
    parser.add_argument('--threshold', type=float, default=0.1, help='Confidence threshold for considering a target clip as a split (confidence = 1 / (1 + |offset|))')
    ...
    # Collect candidate pairs (target_end_idx, source_end_idx, confidence) where confidence > threshold
    candidates = []
    for end_idx in tqdm(candidate_end_indices, desc="Processing target frames"):
        ...
        for source_end_idx, source_clip in source_samples.items():
            with torch.no_grad():
                ...
                offset = output.item()
                confidence = 1.0 / (1.0 + abs(offset))
            if confidence > args.threshold:
                candidates.append((end_idx, source_end_idx, confidence))

    # Get the sorted list of source_end_indices
    source_end_indices = sorted(source_samples.keys())

    predicted_splits = []
    last_target_idx = -1

    for s_idx in source_end_indices:
        # Find candidates for this source split
        split_candidates = [c for c in candidates if c[1] == s_idx]
        # Filter valid candidates with target_end_idx > last_target_idx
        valid_candidates = [c for c in split_candidates if c[0] > last_target_idx]
        if valid_candidates:
            # Select the one with the highest confidence
            best_candidate = max(valid_candidates, key=lambda x: x[2])
            target_end_idx, source_end_idx, confidence = best_candidate
            ...
            last_target_idx = target_end_idx
