...
def generate_training_samples(seed, v1_indices, v1_labels, v1_rider_id, v1_track_id,
                             ...
                             clip_length,
                             num_offset_samples=5, offset_range=50,
    ...
    for split_number in range(start_split, len(v1_split_indices)):
        ...
        # Offset samples
        for _ in range(num_offset_samples):
            offset = rng.integers(-offset_range, offset_range + 1)
            v2_idx = v2_true_idx + offset
            if clip_length - 1 <= v2_idx < v2_total_frames:
                label = -offset  # Label is the offset to align v2_idx back to v2_true_idx
                sample_labels.append(float(label))
                sample_indices.append((v1_split_idx, v2_idx))
                sample_metadata.append({**metadata_base, 'sample_type': 'split_offset'})
    
    # Augmented positive samples for segments
    ...
    for seg in range(num_segments):
        ...
        for idx1 in selected_idx1:
            ...
            fraction_through_v1_segment = (idx1 - v1_split_indices[seg]) / (v1_split_indices[seg + 1] - v1_split_indices[seg])
            v2_true_idx_float = v2_split_indices[seg] + fraction_through_v1_segment * (v2_split_indices[seg + 1] - v2_split_indices[seg])
            ...
            for _ in range(num_offset_samples):
                ...
                if clip_length - 1 <= v2_idx < v2_total_frames:
                    label = -offset  # Label is the offset to align v2_idx back to v2_true_idx
                    sample_labels.append(float(label))
                    sample_indices.append((idx1, v2_idx))
                    sample_metadata.append({**metadata_base, 'sample_type': 'augmented_offset'})
...
def main():
    ...
    parser.add_argument("--num_offset_samples", type=int, default=5, help="Number of offset samples per positive sample")
    parser.add_argument("--offset_range", type=int, default=50, help="Range of frame offsets for samples")
    ...
    for track_id, track_videos_list in track_videos.items():
        ...
        for video1, video2 in video_pairs:
            ...
            sample_labels, sample_indices, sample_metadata = generate_training_samples(
                ...
                num_offset_samples=args.num_offset_samples,
